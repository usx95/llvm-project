...
void placement_new_pointer_field_use_after_scope() {
  ...
  {
    MyObj obj;
    p = new (&h) PointerFieldHolder{&obj}; // expected-warning {{object whose reference is captured does not live long enough}}
  } // expected-note {{destroyed here}}
  (void)p->Ptr->id; // expected-note {{later used here}}
}
...
void foo() {
  ...
  {
    int num;
    s.p_ = &num; // expected-warning {{object whose reference is captured does not live long enough}}
  }              // expected-note {{destroyed here}}
